...
import com.screenlib.utils.MakeLineView;
 ...
 * @author Hathibelagal
 */
public class MainServiceBook extends Service {
    private ImageView mView;
    private View toplayoutView;
    private MakeLineView drawableView;
    ...
    private void reflushView() {
        ...
        if (shared == null) {
            lp = new RelativeLayout.LayoutParams(new WindowManager.LayoutParams(WindowManager.LayoutParams.MATCH_PARENT,
...
//                    PixelFormat.TRANSLUCENT);
        ...
        toplayoutView = LayoutInflater.from(this).inflate(R.layout.topmakelineview, null);
        mView = toplayoutView.findViewById(R.id.imageView);
        drawableView = toplayoutView.findViewById(R.id.drawableView);
        drawableView.cleanView();
        reflushImageView();
        try {
            wm.addView(toplayoutView, params);
        } catch (Exception e) {
            e.printStackTrace();
