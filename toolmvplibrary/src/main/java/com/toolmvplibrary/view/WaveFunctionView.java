...
public class WaveFunctionView extends View {
    ...
    private float waveDeepMax = 40f;//最大的波峰与波谷
    private float waveDeep = 15f;//波峰与波谷
    private float arcRa = 0;//圆半径
    private Boolean iscircle = false;//是否是圆形图案

    private Boolean antiAlias = true;//是否开启抗锯齿

    public String MAINCOLOR_DEF = "#FFFFFF", NEXTCOLOR_DEF = "#96FFFFFF";//默认颜色
...
//    private Double anglenum = Math.PI / 180;
    private Double anglenum = Math.PI / 90;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Double rightperiod = Math.PI / 4 * count;//每次平移Math.PI/8个周期
        if (count == 8) {//每次平移Math.PI/8个周期,平移第16次,平移了一个完整的周期
