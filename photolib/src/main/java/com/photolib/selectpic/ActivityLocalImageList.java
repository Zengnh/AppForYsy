 ...
 * 使用ToolToSelectPic 进行控制，设置选择多少张，等参数设置
 **/
public class ActivityLocalImageList extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setStatusBar();
        setContentView(R.layout.activity_pic_location_image);
        configSelect = ToolToSelectPic.getConfig(getIntent());
        ToolLocationImage.getInstance().setFloag(configSelect.showPic,
...
//        发送/完成
        if (configSelect.fromType == 1) {
            ...
            tvRight = titleView.setRight("发送", new View.OnClickListener() {
                @Override
            ...
            tvRight = titleView.setRight("确定", new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    selectFinish();
                }
    ...
    public void changeState(ArrayList<ImageBean> list) {
        if (configSelect.fromType == 1) {
            ...
            tvRight.setText(list.size() + "/" + configSelect.limit + "确定");
        }
        if (list.size() > 0) {
            tvRight.setAlpha(1f);
...
//        ArrayList<ImageBean> imgs = ToolLocationImage.getInstance().getPicked();
    ...
    public void showToast(String str) {
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                if (toast == null) {
                    toast = Toast.makeText(ActivityLocalImageList.this, str, Toast.LENGTH_SHORT);
